...
export async function createNotification(
  ...
  const notification = await prisma.notification.create({
    data: {
      ...
      priority,
      data: request.data || {},
      actions: request.actions ? JSON.parse(JSON.stringify(request.actions)) : undefined,
      read: false,
      dismissible: true,
    },
  });
...
export async function getNotifications(
  ...
  const notificationResponses: Notification[] = returnNotifications.map((n) => ({
    ...
    actions: n.actions as unknown as NotificationAction[] | undefined,
    read: n.read,
